...
Future<void> populateAll() async {
  ...
  final pilots = ['pilot1@gmail.com', 'pilot2@gmail.com', 'pilot3@gmail.com','pilot4@gmail.com'];
  final now = DateTime.now();

  // Populate pilot metrics
  for (var pilotId in pilots) {
await _firestore.collection('pilotMetrics').doc(pilotId).set({
  'totalFlightHoursLast7Days': 0 + Random().nextInt(25),
  'totalFlightHoursLast28Days': 0 + Random().nextInt(100),
  'timeZonesCrossedLast24Hours': Random().nextInt(12),
  ...
  for (var pilotId in pilots) {
    for (int i = 0; i < 5; i++) {
      ...
      final duration = 0 + Random().nextInt(16);       
  ...
  for (var pilotId in pilots) {
    for (int i = 0; i < 5; i++) {
      ...
      final duration = 0 + Random().nextInt(14); // 10-14 hours rest periods
  ...
  for (var flightDoc in flightDocs.docs) {
   ...
   final dutyHourScore = 0 + Random().nextInt(100);
final timezoneScore = 0 + Random().nextInt(100);
final restPeriodScore = 0 + Random().nextInt(100);
final flightDurationScore = 0 + Random().nextInt(100);
final selfAssessmentScore = 0 + Random().nextInt(100);
